...
public class AnalizadorCiberseguridad {
    ...
    public static void main(String[] args) {
        ...
        for(Log log : logsUnicos){
            ...
            ataquesPorIP.putIfAbsent(ip,1);
            ataquesPorIP.put(ip, ataquesPorIP.get(ip));
        }
        ...
        List<Log> logsCriticos = new ArrayList<>();
        List<Log> logsMediaos  = new ArrayList<>();
        List<Log> logsBajos    = new ArrayList<>();

        for (Log log : logsUnicos) {
            String nivelRiesgo = nivelRiesgoZona.get(log.ubicacion);

            if (nivelRiesgo.equals("CRÍTICO")) {
                logsCriticos.add(log);
            } else if (nivelRiesgo.equals("MEDIO")) {
                logsMediaos.add(log);
            } else if (nivelRiesgo.equals("BAJO")) {
                logsBajos.add(log);
            }
        }
        ...
        for (Log log : logsBrutos){
            if(log instanceof LogCamara){
                ...
                idsRevisionVisual.add(camara.idLog);
            }
        }
